    ...
    toJSON: function () {
        ...
        return JSON.stringify(data, function (key, value) {
            ...
            if (value instanceof MatArray) {
                ...
                if (value.type === 'mxCELL_CLASS') {
                    obj[value.name] = data.map(v=>v.data)
                    return obj
                }
                obj[value.name] = data;
                return obj;
...
MatArray.prototype = Object.assign( Object.create( MatObject.prototype ) , {
    ...
    parse: function (data) {
        ...
        switch (ARRAYTYPE[ArrayFlag[0]]){
            case 'mxSTRUCT_CLASS':
                ...
                break;
            case 'mxCELL_CLASS':
                this.data = this.parseCell(data.slice(offset));
                break;
            default:
    ...
    parseArray: function (data, DataArray) {
        ...
        switch (data_type){
            ...
            case 'miUINT32':
                return arrayBufferToArray(new Uint32Array(data_block));
            case 'miSINGLE':
                throw 'don\'t support type as miSINGLE';// 不会
            case 'miDOUBLE':
    ...
    parseCell: function (data) {
        ...
        for (let i = 0;i < data.byteLength;) {
            ...
            if (DATATYPE[data_info[0]] === 'null') throw 'parse cell error'
            switch (DATATYPE[data_info[0]]){
                case 'miMATRIX':
                    data_array.push(new MatArray(data_block))
                    data_array[data_array.length-1].name = data_array.length
                    break;
                default:
                    throw 'parse cell error';
            }
        }
        return data_array
    }
